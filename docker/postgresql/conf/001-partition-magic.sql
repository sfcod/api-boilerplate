CREATE OR REPLACE FUNCTION partition_magic_before_insert_trigger() RETURNS trigger AS $$
DECLARE
	hasMeta boolean;
	meta RECORD;
...
BEGIN
	hasMeta := false;
	FOR meta IN SELECT * FROM partition_magic_meta m WHERE m.table_name = TG_TABLE_NAME
	LOOP
		hasMeta := true;
	END LOOP;
	...
	IF hasMeta THEN
		...
		IF ( NOT EXISTS ( SELECT 1 FROM pg_tables t WHERE t.schemaname = meta.schema_name AND t.tablename = itable ) ) THEN
			partitionRes := partition_magic(meta.parent_table_name, meta.action_field, partition_id, meta.schema_name, meta.partition_table_prefix, FALSE);
		END IF;
...
CREATE OR REPLACE FUNCTION partition_magic_after_insert_trigger() RETURNS trigger AS $$
DECLARE
	hasMeta boolean;
	meta RECORD;
	itable text;
...
CREATE OR REPLACE FUNCTION partition_magic(parent_table text, action_field text, partition_idx integer = NULL, schema_name text = NULL, partition_table_prefix text = NULL, is_debug boolean = FALSE) RETURNS boolean AS $$
DECLARE
	itable varchar(255);
...
BEGIN
	...
	IF ( NOT EXISTS ( SELECT 1 FROM pg_tables t WHERE t.schemaname = schema_name AND t.tablename = 'partition_magic_meta' ) ) THEN
		IF(is_debug) THEN RAISE INFO '----- [Creating META table "%"] -----', 'partition_magic_meta'; END IF;
		EXECUTE 'CREATE TABLE partition_magic_meta (id integer, table_name character varying(255), action_field character varying(255), partition_id integer, schema_name character varying(255), partition_table_prefix character varying(255), parent_table_name character varying(255), created_at TIMESTAMP DEFAULT NOW());';
		EXECUTE 'CREATE INDEX table_name_idx ON partition_magic_meta (table_name);';
		EXECUTE 'CREATE INDEX partition_id_idx ON partition_magic_meta (partition_id);';
		EXECUTE 'CREATE INDEX parent_table_name_idx ON partition_magic_meta (parent_table_name);';
	END IF;

	IF ( NOT EXISTS ( SELECT 1 FROM partition_magic_meta m WHERE m.table_name = parent_table ) ) THEN
		IF(is_debug) THEN RAISE INFO '----- [Creating META for table "%.%"] -----', schema_name, parent_table; END IF;
		EXECUTE 'INSERT INTO partition_magic_meta (table_name, action_field, partition_id, schema_name, partition_table_prefix, parent_table_name) VALUES (''' || parent_table || ''', ''' || action_field || ''', NULL, ''' || schema_name || ''', ''' || partition_table_prefix || ''', ''' || parent_table || ''');';
	END IF;
	...
	IF ( NOT EXISTS ( SELECT g.tgfoid::regclass::text, pg_get_functiondef(p.oid) as procdef, prosrc, pg_get_triggerdef(g.oid) as tgdef, g.tgname
										...
										WHERE g.tgrelid::regclass::text = parent_table AND g.tgname = 'partition_magic_before_insert_' || parent_table AND g.tgenabled != 'D' AND NOT g.tgisinternal ) )
	THEN
		IF(is_debug) THEN RAISE INFO '----- [Creating before insert trigger on parent_table "%.%"] -----', schema_name, parent_table; END IF;
		EXECUTE 'CREATE TRIGGER partition_magic_before_insert_' || parent_table || ' BEFORE INSERT ON ' || parent_table || ' FOR EACH ROW EXECUTE PROCEDURE partition_magic_before_insert_trigger();';
		IF(is_debug) THEN RAISE INFO '----- [Creating after insert trigger on parent_table "%.%"] -----', schema_name, parent_table; END IF;
		EXECUTE 'CREATE TRIGGER partition_magic_after_insert_' || parent_table || ' AFTER INSERT ON ' || parent_table || ' FOR EACH ROW EXECUTE PROCEDURE partition_magic_after_insert_trigger();';
	END IF;
	...
	IF(partition_idx IS NULL) THEN
		...
		LOOP
			...
			res := res AND partition_magic(parent_table, action_field, partition_idx, schema_name, partition_table_prefix, is_debug);
		END LOOP;
	...
	IF ( NOT EXISTS ( SELECT 1 FROM pg_tables t WHERE t.schemaname = schema_name AND t.tablename = itable ) ) THEN
		...
		EXECUTE 'INSERT INTO partition_magic_meta (table_name, action_field, partition_id, schema_name, partition_table_prefix, parent_table_name) VALUES (''' || itable || ''', ''' || action_field || ''', ' || partition_idx || ', ''' || schema_name || ''', ''' || partition_table_prefix || ''', ''' || parent_table || ''');';
	END IF;
	...
	LOOP
		...
		IF idx1_name = 'partition_magic_before_insert_' || parent_table THEN
			CONTINUE;
		END IF;

		IF idx1_name = 'partition_magic_after_insert_' || parent_table THEN
			CONTINUE;
		END IF;
